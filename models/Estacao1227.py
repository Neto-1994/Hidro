...
try:
    class Estacao1227():
        def _init_(self, data1, data2, Nome_Arquivo, Nome_Salvar):
            consulta_sql = "SELECT DATE(HoraLocal), AVG(SPressao), AVG(Vazao_calculada) FROM medicoes WHERE Codigo_Sec = 1227 AND HoraLocal between %s and %s GROUP BY DATE(HoraLocal);"
