...
try:
    class Estacao1230():
        def _init_(self, data1, data2, Nome_Arquivo, Nome_Salvar):
            consulta_sql = "SELECT DATE(HoraLocal), AVG(SPressao), AVG(Vazao_calculada) FROM medicoes WHERE Codigo_Sec = 1230 AND HoraLocal between %s and %s GROUP BY DATE(HoraLocal);"
