...
try:
    class Estacao1244():
        def _init_(self, data1, data2, Nome_Arquivo, Nome_Salvar):
            consulta_sql = "SELECT DATE(HoraLocal), AVG(SPressao), AVG(Vazao_calculada) FROM medicoes WHERE Codigo_Sec = 1244 AND HoraLocal between %s and %s GROUP BY DATE(HoraLocal);"
